/* GStreamer libspotify source
 * Copyright (C) 2009 Joel Larsson
 *                    Johan Gyllenspetz
 *               2012 Sean McNamara <smcnam@gmail.com>
 *
 * gstspotsrc.c:
 *
...
#define DEFAULT_USER_AGENT "spotify-gstreamer-src"
#define DEFAULT_DEVICE_ID (g_get_host_name())
#define DEFAULT_TRACEFILE NULL
...
run_spot_cmd (GstSpotSrc *spot, enum spot_cmd cmd, guint64 *retval, gint64 opt, void *userdata)
{
  ...
  g_cond_init(&(spot_work->spot_cond));
  g_mutex_init(&(spot_work->spot_mutex));
  spot_work->cmd = cmd;
  spot_work->retval = 0;
  ...
  *retval = spot_work->retval;
  ...
  /* remove work struct */
  g_cond_clear (&(spot_work->spot_cond));
  g_mutex_clear (&(spot_work->spot_mutex));
  g_free (spot_work);
...
static gboolean spotify_create_session (GstSpotSrc *spot)
{
  ...
  if(g_file_get_contents(GST_SPOT_SRC_SPOTIFY_KEY_FILE (spot), &appkey, &appkey_size, &err) == FALSE) {
    GST_ERROR_OBJECT (spot, "Failed to open spot keyfile: %s\n", 
        GST_SPOT_SRC_SPOTIFY_KEY_FILE (spot));
    return FALSE;
  }

  appkey_size--; //Don't pull in the NUL byte at the end of the buffer

  config.api_version = SPOTIFY_API_VERSION;
  config.cache_location = g_get_tmp_dir();
  config.settings_location = g_get_tmp_dir();
  config.application_key = appkey;
  config.application_key_size = appkey_size;
  config.user_agent = spot->user_agent;
...
static gboolean spotify_login (GstSpotSrc *spot)
{
  ...
  error = sp_session_relogin(GST_SPOT_SRC_SPOTIFY_SESSION (spot));
  if(error == SP_ERROR_NO_CREDENTIALS) {
    /* login using the credentials given on the command line */
    error = sp_session_login (GST_SPOT_SRC_SPOTIFY_SESSION (spot), GST_SPOT_SRC_USER (spot), GST_SPOT_SRC_PASS (spot), spot->remember, NULL);
  }
...
spotify_thread_func (void *data)
{
  ...
  while (spot->keep_spotify_thread) {
    ...
    g_cond_wait_until (&(spot->process_events_cond), &(spot->process_events_mutex), end_time);
    ...
    while (spot->spot_works) {
        ...
        case SPOT_CMD_SEARCH:
	  {
	    ...
	    search = sp_search_create(GST_SPOT_SRC_SPOTIFY_SESSION (spot), s_s->query,
				      s_s->track_index, s_s->track_nbr, s_s->album_index,
				      s_s->album_nbr, s_s->artist_index, s_s->artist_nbr, s_s->playlist_index,
              s_s->playlist_nbr, SP_SEARCH_STANDARD, &spotify_cb_search_complete, s_s);
...
gst_spot_src_class_init (GstSpotSrcClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, ARG_REMEMBER,
    g_param_spec_boolean ("remember-me", "Remember Me", "Whether to remember credentials or attempt to relogin using existing credentials",
          DEFAULT_REMEMBER, G_PARAM_READWRITE));

  g_object_class_install_property (gobject_class, ARG_USER_AGENT,
    g_param_spec_string ("user-agent", "User Agent", "The user agent to provide to spotify. Usually the name of the application",
          DEFAULT_USER_AGENT, G_PARAM_READWRITE));

  g_object_class_install_property (gobject_class, ARG_DEVICE_ID,
    g_param_spec_string ("device-id", "Device ID", "The device ID to provide to spotify",
          DEFAULT_DEVICE_ID, G_PARAM_READWRITE));

  g_object_class_install_property (gobject_class, ARG_TRACEFILE,
    g_param_spec_string ("tracefile", "Tracefile Path", "File path to trace file for libspotify API",
          DEFAULT_TRACEFILE, G_PARAM_READWRITE));
...
gst_spot_src_init (GstSpotSrc * spot, GstSpotSrcClass * g_class)
{
  ...
  /* its childish to use static global variables - so Joel and Johan are obviously children ;)*/
  ...
  g_mutex_init(GST_SPOT_SRC_ADAPTER_MUTEX (spot));
  g_cond_init(GST_SPOT_SRC_ADAPTER_COND (spot));
  ...
  g_mutex_init(&(spot->process_events_mutex));
  g_cond_init(&(spot->process_events_cond));
  spot->process_events_thread = g_thread_new ("gstspotsrc", (GThreadFunc)spotify_thread_func, spot);

  if (spot->process_events_thread == NULL) {
     GST_CAT_ERROR_OBJECT (gst_spot_src_debug_threads, spot,"g_thread_new failed!" );
     //g_error_free (err) ;
  }
...
gst_spot_src_finalize (GObject * object)
{
  ...
  g_cond_clear (&(spot->process_events_cond));
  g_mutex_clear (&(spot->process_events_mutex));

  g_mutex_clear (GST_SPOT_SRC_ADAPTER_MUTEX (spot));
  g_cond_clear (GST_SPOT_SRC_ADAPTER_COND (spot));
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case ARG_TRACEFILE:
      g_free(spot->tracefile);
      spot->tracefile = g_value_dup_string(value);
      break;
    case ARG_DEVICE_ID:
      if(g_strcmp0(DEFAULT_DEVICE_ID, g_value_get_string(value)) != 0)
        g_free(spot->device_id);
      spot->device_id = g_value_dup_string(value);
      break;
    case ARG_USER_AGENT:
      if(g_strcmp0(DEFAULT_USER_AGENT, g_value_get_string(value)) != 0)
        g_free(spot->user_agent);
      spot->user_agent = g_value_dup_string(value);
      break;
    case ARG_REMEMBER:
      spot->remember = g_value_get_boolean(value);
      break;
    case ARG_USER:
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case ARG_TRACEFILE:
      g_value_set_string(value, spot->tracefile);
      break;
    case ARG_DEVICE_ID:
      g_value_set_string(value, spot->device_id);
      break;
    case ARG_USER_AGENT:
      g_value_set_string(value, spot->user_agent);
      break;
    case ARG_REMEMBER:
      g_value_set_boolean (value, spot->remember);
      break;
    case ARG_USER:
